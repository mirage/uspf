...
let get : type dns. domain:_ Domain_name.t -> dns -> (module DNS with type t = dns) -> _ =
 fun ~domain:domain_name dns (module DNS) ->
  ...
  | Ok (_, txts) ->
    match R.(Spf.select_spf1 (Dns.Rr_map.Txt_set.elements txts) >>= Spf.Term.parse_record) with
