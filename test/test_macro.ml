...
let test01 =
  ...
  Alcotest.(check (result string msg))
    "01"
    (Spf.Macro.expand_string ctx "%{s}")
    (Ok "strong-bad@email.example.com") ;
  Alcotest.(check (result string msg))
    "02"
    (Spf.Macro.expand_string ctx "%{o}")
    (Ok "email.example.com") ;
  Alcotest.(check (result string msg))
    "03"
    (Spf.Macro.expand_string ctx "%{d}")
    (Ok "email.example.com") ;
  Alcotest.(check (result string msg))
    "04"
    (Spf.Macro.expand_string ctx "%{d4}")
    (Ok "email.example.com") ;
  Alcotest.(check (result string msg))
    "05"
    (Spf.Macro.expand_string ctx "%{d3}")
    (Ok "email.example.com") ;
  Alcotest.(check (result string msg))
    "06"
    (Spf.Macro.expand_string ctx "%{d2}")
    (Ok "example.com") ;
  Alcotest.(check (result string msg))
    "07"
    (Spf.Macro.expand_string ctx "%{d1}")
    (Ok "com") ;
  Alcotest.(check (result string msg))
    "08"
    (Spf.Macro.expand_string ctx "%{dr}")
    (Ok "com.example.email") ;
  Alcotest.(check (result string msg))
    "09"
    (Spf.Macro.expand_string ctx "%{d2r}")
    (Ok "example.email") ;
  Alcotest.(check (result string msg))
    "10"
    (Spf.Macro.expand_string ctx "%{l}")
    (Ok "strong-bad") ;
  Alcotest.(check (result string msg))
    "11"
    (Spf.Macro.expand_string ctx "%{l-}")
  ...
  Alcotest.(check (result string msg))
    "12"
    (Spf.Macro.expand_string ctx "%{lr}")
    (Ok "strong-bad") ;
  Alcotest.(check (result string msg))
    "13"
    (Spf.Macro.expand_string ctx "%{lr-}")
  ...
  Alcotest.(check (result string msg))
    "14"
    (Spf.Macro.expand_string ctx "%{l1r-}")
    (Ok "strong")
